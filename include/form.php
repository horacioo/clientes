...
add_shortcode("nome", function() {
    $x = "\n<label>Nome</label><input required='required' id='nome' type='text' name=dados[clientes][nome] class='form-control'>";
    return $x;
});
...
add_shortcode("cpf", function() {
    $x = "\n<label>Cpf</label><input type='text' required='required' id='cpf' name=dados[clientes][cpf] class='form-control'>";
    return $x;
});


add_shortcode("rg", function() {
    $x = "\n<label>Rg</label><input type='text' required='required' id='rg' name=dados[clientes][rg] class='form-control'>";
    return $x;
});


add_shortcode("data_de_expedicao", function() {
    $x = "\n<label>Data de expedição</label><input required='required' type='date' id='data_de_expedicao' name=dados[clientes][dataExpedicao] class='form-control'>";
    return $x;
});


add_shortcode("data_de_nascimento", function() {
    $x = "\n<label>Data de Nascimento</label><input required='required' id='nascimento' type='date' name=dados[clientes][dataNascimento] class='form-control'>";
    return $x;
});
...
add_shortcode("interesse", function($atts, $content = '') {
    if (!empty($content)) {
        $x   = "<label>Interesse</label>";
        $x   .= "\n<select name=dados[interesse]>";
        $x   .= "<option value='#'>gostaria de mais informações sobre:</option>";
        $opt = explode(",", $content);

        foreach ($opt as $op):
            $x .= "<option value='".trim($op)."'>$op</option>";
        endforeach;
