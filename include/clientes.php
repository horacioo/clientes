...
class clientes
    ...
    public static function ClientesEnvioEmail() {
        ...
        $Sel     = "SELECT id,nome FROM `clientes` where id not in($sel2) LIMIT 1";
        $dados   = $wpdb->get_results($Sel, ARRAY_A);
        $cliente = array();
        foreach ($dados as $d):
            $cliente[] = array("nome" => $d['nome'], "id" => $d['id']);
    ...
    public static function ClientesNovos() {
        ...
        $sel      = "select id, nome from clientes where entrada between '$anterior' and '$hoje'  LIMIT 1";
        $dados    = $wpdb->get_results($sel, ARRAY_A);
        $cliente  = array();
        foreach ($dados as $d):
            $cliente[] = array("nome" => $d['nome'], "id" => $d['id']);
    ...
    public static function ClientesRecebemEmailsAposDias($dias = "") {
        ...
        $select = "select cl.id, cl.nome from clientes as cl where cl.id not in($Sel) and  cl.entrada between '$dias1' and '$dias2'group by cl.id  LIMIT 1 ";
